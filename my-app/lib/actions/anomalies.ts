...
export type CreateAnomalyData = {
  ...
  report_date?: string // por defecto hoy (YYYY-MM-DD)
}
...
export async function createAnomaly(data: CreateAnomalyData) {
  ...
  try {
    ...
    // Si es crítica/alta, ponemos la unidad en mantenimiento
...
export async function resolveAnomaly(id: string, payload: { actual_repair_cost: number; resolution_notes: string }) {
  const supabase = createServerClient()
  try {
    // obtener anomaly para saber la unidad
    ...
    const { data: row, error } = await supabase
      ...
      .update({
        ...
        actual_repair_cost: payload.actual_repair_cost,
    ...
    // Si ya no quedan abiertas/en_proceso críticas/altas de esa unidad, volver a activo
    if (curr?.unit_id) {
...
export async function getAnomaliesSummary(filters?: { date_from?: string; date_to?: string }) {
  ...
  try {
    let query = supabase
      .from("anomaly_reports")
      .select("status, severity, estimated_repair_cost, actual_repair_cost")
    ...
    const list =
      (data as { status: Anomaly["status"]; severity: Anomaly["severity"]; estimated_repair_cost: number | null; actual_repair_cost: number | null }[]) ??
      []
...
/** Para select de unidades en el formulario */
