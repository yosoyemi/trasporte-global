...
// Tipado flexible para normalizar filas de fuel (evita any)
type FuelTrendLike = {
  month?: unknown;
  liters?: unknown;
  cost?: unknown;
  ...
  lph?: unknown;
  hours?: unknown;
  hours_operated?: unknown;
  records?: unknown;
  count?: unknown;
};

// Normaliza las filas que vengan de getFuelTrends() a FuelTrend
function normalizeFuelTrends(rows: unknown[]): FuelTrend[] {
  const list = Array.isArray(rows) ? (rows as FuelTrendLike[]) : [];
  return list.map((r) => {
    const litersNum = Number((r && r.liters) ?? 0);
    const costRaw = (r && (r.cost ?? r.usd ?? r.fuel_cost)) as unknown;
    const costNum = Number(costRaw ?? 0);

    const hoursNum = Number((r && (r.hours ?? r.hours_operated)) ?? 0);
    const efficiencyRaw = (r && (r.efficiency ?? r.lph)) as unknown;
    const efficiencyNum = Number(
      Number.isFinite(Number(efficiencyRaw)) ? efficiencyRaw : hoursNum > 0 ? litersNum / hoursNum : 0
    );

    const monthStr = String((r && r.month) ?? "");
    const recordsNum = Number((r && (r.records ?? r.count)) ?? 0);

    return {
      month: monthStr,
      liters: Number.isFinite(litersNum) ? litersNum : 0,
      cost: Number.isFinite(costNum) ? costNum : 0,
      efficiency: Number.isFinite(efficiencyNum) ? efficiencyNum : 0,
      records: Number.isFinite(recordsNum) ? recordsNum : 0,
    };
  });
}
...
export default async function Dashboard() {
  ...
  const fuelData: FuelTrend[] = fuelResult.success
    ? normalizeFuelTrends(fuelResult.data as unknown[])
    : [];
